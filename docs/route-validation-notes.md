...
The Collections workspace now renders an explicit local hydration skeleton before the valuation, purchase, and Commander Passport modules mount. Shared card/product image renderers normalize empty URLs, expose loading state, and fall back to direct Scryfall artwork when boxed product photography is unavailable.

The recent-set, Commander archive, and set-detail hooks now bound their initial Scryfall requests and turn timeouts into recoverable messages instead of leaving an indefinite skeleton. `SetDetail` also keeps its local precon-card memo above loading/error returns, eliminating the hook-order crash observed when `/eoe` transitioned from its skeleton to the catalog. Related Commander child-set codes are included when building parent-set precon attachments.

Desktop and mobile captures covered `/collection`, `/eoe`, and `/precons`. A controlled Chromium DOM check waited seven seconds on `/eoe` and confirmed the catalog rendered 399 cards, two associated Commander precons, and loaded Scryfall/manus-storage artwork with non-zero natural dimensions.
