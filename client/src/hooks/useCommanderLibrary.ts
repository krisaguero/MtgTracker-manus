  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        try {
          response = await fetch('https://api.scryfall.com/sets', { signal: initialController.signal });
        } finally {
        ...
        const historical = (payload.data || [])
          .filter((set) => set.set_type === 'commander' && Boolean(set.released_at) && set.released_at! >= '2011-01-01' && set.released_at! <= today)
        ...
        if (!active) return;
        const timedOut = isAbortError(loadError);
        setError(timedOut ? 'The Commander archive took too long to respond. Please retry.' : loadError instanceof Error ? loadError.message : 'Unable to load Commander archive');
        setLoading(false);
