...
type SnapshotProduct = { id: string; set_code: string; name: string; usd: string | number | null; scryfall_uri?: string };
...
function priceForCard(card: RawDeckCard, setCode: string) {
  ...
  return product ? Number(product.usd || 0) : 0;
}
...
export function getMarketSpikes(): MarketSpikeItem[] {
  ...
  const items: MarketSpikeItem[] = [];
  for (const deck of commanderDecklistsData) {
    for (const card of deck.cards) {
      const product = productByName.get(`${deck.set_code.toLowerCase()}::${card.name.toLowerCase()}`) || snapshotProducts.find((entry) => entry.name.toLowerCase() === card.name.toLowerCase());
      const price = Number(product?.usd || 0);
      if (price < 3.0) continue;
      const previousUsd = product?.id && previousPrices[product.id] !== undefined ? Number(previousPrices[product.id]) : price;
      const changeUsd = Number((price - previousUsd).toFixed(2));
      const percentChange = previousUsd > 0 ? Number(((changeUsd / previousUsd) * 100).toFixed(1)) : 0;
      items.push({
        cardName: card.name,
        format: commanderCodes.has(deck.set_code.toLowerCase()) ? 'Commander' : 'Standard',
        setName: deck.set_name,
        setCode: deck.set_code,
        rarity: card.number === '1' ? 'mythic' : 'rare',
        currentUsd: price,
        previousUsd,
        changeUsd,
        percentChange,
        isNewRelease: card.number !== '150' && card.number !== '180',
        scryfallUri: product?.scryfall_uri || `https://scryfall.com/search?q=${encodeURIComponent(card.name)}`,
      });
    }
  }
  ...
  for (const item of items) {
    if (!uniqueMap.has(item.cardName) || uniqueMap.get(item.cardName)!.currentUsd < item.currentUsd) {
      uniqueMap.set(item.cardName, item);
    }
  }
