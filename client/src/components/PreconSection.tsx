...
function PreconCard({
  ...
  const cardMarkup = (
    <>
      <div className="flex h-48 w-full items-center justify-center bg-muted/40 p-4" title={precon.productImageSourceLabel || 'Commander product artwork'}>
        {precon.productImageUrl || precon.image_uris?.normal ? (
          <img 
            ...
            loading="lazy" 
            onError={(e) => {
              (e.currentTarget as HTMLImageElement).src = `data:image/svg+xml;charset=UTF-8,<svg xmlns="http://www.w3.org/2000/svg" width="240" height="240" viewBox="0 0 240 240"><rect fill="%231e293b" width="240" height="240"/><text x="120" y="110" font-family="monospace" font-size="13" fill="%23cbd5e1" text-anchor="middle">${encodeURIComponent(precon.name)}</text><text x="120" y="135" font-family="monospace" font-size="10" fill="%2394a3b8" text-anchor="middle">Commander Precon</text></svg>`;
            }}
          />
        ) : (
