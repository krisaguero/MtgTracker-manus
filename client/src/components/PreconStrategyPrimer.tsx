...
export function PreconStrategyPrimer({
  ...
  if (approxValue > 120) powerLevel = 'Bracket 3 / High Synergy Casual (Power 7-8)';
  if (approxValue < 45) powerLevel = 'Bracket 1 / Out-of-the-Box Precon (Power 5-6)';
    ...
    <div className="space-y-8 my-10">
      ...
      <section className="border-2 border-border bg-card p-6 sm:p-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          {topValueCards.map((entry) => {
            const img = entry.card.image_uris?.normal || entry.card.card_faces?.[0]?.image_uris?.normal;
            const price = entry.card.prices?.usd ? Number(entry.card.prices.usd) : 0;

            return (
              <div key={entry.card.id} className="border-2 border-border bg-background p-3 flex flex-col justify-between">
                <div>
                ...
                <div className="mt-4 pt-2 border-t border-border flex items-center justify-between font-mono text-xs font-bold text-primary">
                  <span>Market:</span>
