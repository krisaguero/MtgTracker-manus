// Design philosophy: hard-edged collection dashboard showing market equity by set, historical value tracking chart, and coverage-ranked deck recommendations.
...
import { getDeckCoverageRecommendations, type DeckCoverageRecommendation } from '@/lib/collectionAnalytics';
import { Button } from '@/components/ui/button';
import { Link } from 'wouter';
import { Wallet, Sparkles, Upload, Layers, TrendingUp } from 'lucide-react';
...
export function CollectionOverview({ onOpenImportModal, refreshKey }: CollectionOverviewProps) {
  ...
  useEffect(() => {
    ...
    // Approximate total value based on indexed approximate deck values or default $1.50 per card for unpriced cards
    const estimatedValue = col.reduce((sum, c) => sum + c.quantity * 2.0, 0);
    if (col.length > 0) {
      const updatedHistory = recordValuationSnapshot(estimatedValue);
  ...
  const currentValuation = history.length > 0 ? history[history.length - 1]!.estimatedValueUsd : 0;
      ...
      {hasCollection ? (
        <div className="mt-8 space-y-8">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="border border-border bg-background p-5">
              <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Estimated Market Equity</p>
              <p className="mt-2 text-3xl font-extrabold text-primary">${currentValuation.toLocaleString()} <span className="text-sm font-normal text-muted-foreground">USD</span></p>
              <p className="mt-1 text-xs text-muted-foreground">Based on Scryfall price index averages.</p>
            </div>
          ...
          <div className="border border-border bg-background p-6">
            ...
            <p className="mt-4 text-[11px] text-muted-foreground">Note: Chart reflects private local snapshots. Market equity is an approximate guide and does not represent guaranteed resale liquidity.</p>
          </div>
