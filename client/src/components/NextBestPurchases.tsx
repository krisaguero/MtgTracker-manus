...
export function NextBestPurchases() {
  ...
  const closestDeckData = useMemo(() => {
    ...
    let bestCoverage = -1;
    let missingList: Array<{ name: string; price: number; type: string }> = [];

    for (const deck of commanderDecklistsData) {
      const allCards = [...deck.commander, ...deck.cards];
      let total = 0;
      let owned = 0;
      const missing: Array<{ name: string; price: number; type: string }> = [];
      ...
      for (const card of allCards) {
        ...
        if (ownedQty >= qty) {
          ...
          const estPrice = deck.approxValue ? Math.max(0.5, Number((deck.approxValue / 100).toFixed(2))) : 2.0;
          missing.push({
            name: card.name,
            price: estPrice,
      ...
      if (pct < 100 && pct > bestCoverage) {
        bestCoverage = pct;
        bestDeck = deck;
        missingList = missing.sort((a, b) => a.price - b.price);
      }
    }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b-2 border-border pb-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Based on your uploaded inventory of {collection.length} cards, this precon is closest to completion. Here are the cheapest missing singles to buy next.
          </p>
        </div>
