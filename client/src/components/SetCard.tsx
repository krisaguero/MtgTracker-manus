  ...
  const cardMarkup = (
    <div>
      <div className="flex h-28 items-center justify-center bg-muted/40 p-2 sm:h-36 sm:p-3" title={precon.productImageSourceLabel || 'Commander product artwork'}>
        {precon.productImageUrl || precon.image_uris?.normal ? <img src={precon.productImageUrl || precon.image_uris?.normal} alt={`${precon.name} boxed Commander product`} className="h-full w-full object-contain" loading="lazy" /> : <span className="px-4 text-center text-sm text-muted-foreground">{precon.name}</span>}
      </div>
