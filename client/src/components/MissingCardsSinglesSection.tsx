...
function currentPrice(card?: ScryfallCard) {
  if (!card) return null;
  const indexed = resolveCardPrice(card.name, card.set);
  if (indexed) return indexed.usd;
  const scryfall = card.prices?.usd ? Number(card.prices.usd) : NaN;
  return Number.isFinite(scryfall) && scryfall > 0 ? scryfall : null;
}

export function MissingCardsSinglesSection({ deckName, commanderEntries, deckEntries, sealedApproxValue = null }: MissingCardsSinglesSectionProps) {
  ...
  const singlesTotalCost = useMemo(() => {
    ...
    for (const item of visibleMissing) {
      ...
      const priceNum = currentPrice(match?.card);
      if (priceNum !== null) sum += priceNum * item.missing;
    }
  ...
  const groupedMissing = useMemo(() => {
    const allEntries = [...commanderEntries, ...deckEntries];
    const groups: Array<{ label: string; items: Array<{ item: MissingCardItem; match?: ScryfallCard; unitPrice: number | null; lineTotal: number | null; isExpensive: boolean }> }> = [];
    ...
    for (const cat of CATEGORIES) {
      ...
      for (const item of visibleMissing) {
        ...
        if (cat.test(typeLine)) {
          const unitPrice = currentPrice(match?.card);
          const lineTotal = unitPrice === null ? null : unitPrice * item.missing;
          const isExpensive = unitPrice !== null && unitPrice > 5.00;
  ...
  const unpricedMissingCount = groupedMissing.reduce((count, group) => count + group.items.filter((item) => item.unitPrice === null).length, 0);
  const hasSealedBaseline = typeof sealedApproxValue === 'number' && sealedApproxValue > 0;
    ...
    <section className="relative border-2 border-border bg-card p-6 sm:p-8 my-10 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border pb-5">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          {hasSealedBaseline && singlesTotalCost > sealedApproxValue! && (
          ...
          <div className="border-2 border-primary bg-primary/10 px-5 py-3 text-right shrink-0 flex flex-col justify-center">
            ...
            <span className="block font-mono text-[10px] text-muted-foreground">{visibleMissing.length} unique missing cards · {unpricedMissingCount} unpriced</span>
          </div>
        </div>
      </div>
                  ...
                  <div key={item.name} className={`bg-card p-4 flex flex-col justify-between border-2 ${isExpensive ? 'border-amber-500/70 bg-amber-950/10' : 'border-border'}`}>
                    ...
                    <div className="mt-4 pt-2 border-t border-border flex items-center justify-between font-mono text-xs">
                      <span className={isExpensive ? 'font-bold text-amber-600 dark:text-amber-400' : 'text-muted-foreground'}>
                          Unit: {unitPrice === null ? 'Unavailable' : `$${unitPrice.toFixed(2)}`}
                      </span>
                      <span className="font-bold text-primary">Subtotal: {lineTotal === null ? 'Unavailable' : `$${lineTotal.toFixed(2)}`}</span>
                    </div>
                  </div>
                ))}
              </div>
