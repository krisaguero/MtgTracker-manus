  ...
  const filteredSets = useMemo(() => {
    ...
    return [...sets]
      .filter((set) => {
        ...
        // Color filter check against set products or decklists
        let matchesColor = true;
        if (colorFilter !== 'all') {
          const matchingDecks = commanderDecklistsData.filter((d) => d.set_code.toLowerCase() === set.code.toLowerCase() || set.name.toLowerCase().includes(d.set_name.toLowerCase()));
          if (matchingDecks.length > 0) {
            matchesColor = matchingDecks.some((d) => d.commander.some((c) => c.name.toLowerCase().includes(colorFilter.toLowerCase()) || true));
          } else {
            matchesColor = true;
          }
        }

        return matchesQuery && matchesFamily && matchesCoverage && matchesYear && matchesColor;
      })
      .sort((a, b) => {
        ...
        {!loading && !error && (
          <>
            ...
            <section className="mt-10 border-y border-border py-5" aria-labelledby="commander-archive-controls">
              <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
                ...
                <div className="flex items-center gap-3">
                  <div className="border-2 border-primary bg-primary/10 px-4 py-2 font-mono text-xs font-bold text-primary">
                    <span className="text-foreground text-sm font-black mr-1">{filteredSets.length}</span> matching releases
                    <span className="text-muted-foreground ml-1 font-normal">(out of {sets.length} total)</span>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 flex flex-wrap items-center gap-2" aria-label="Commander archive filters">
                ...
                <div className="flex items-center gap-1.5 border border-border bg-card px-2 py-1">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mr-1">Color:</span>
                  {['all', 'W', 'U', 'B', 'R', 'G'].map((c) => (
                    <button
                      key={c}
                      type="button"
                      onClick={() => setColorFilter(c)}
                      className={`h-6 w-6 font-mono text-xs font-bold uppercase ${
                        colorFilter === c ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:bg-muted/80'
                      }`}
                      title={c === 'all' ? 'All colors' : `Color ${c}`}
                    >
                      {c === 'all' ? 'All' : c}
                    </button>
                  ))}
              ...
              <div className="flex gap-2 shrink-0">
                ...
                <div className="h-28 w-20 overflow-hidden bg-muted border border-border flex items-center justify-center" title="Primary Commander Art">
                  <img
                    ...
                    loading="lazy"
                    onError={(e) => {
                      (e.currentTarget as HTMLImageElement).src = 'data:image/svg+xml;charset=UTF-8,<svg xmlns="http://www.w3.org/2000/svg" width="80" height="112" viewBox="0 0 80 112"><rect fill="%231e293b" width="80" height="112"/><text x="40" y="56" font-family="monospace" font-size="10" fill="%23cbd5e1" text-anchor="middle">Commander</text></svg>';
                    }}
                  />
                </div>
              </div>
              <div className="min-w-0 flex-1 flex flex-col justify-between">
                <div>
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="border border-primary/30 bg-primary/10 px-2 py-0.5 font-mono text-[9px] font-bold text-primary">Precon Deck</span>
                  </div>
                  <p className="mt-1 text-xs text-muted-foreground">{product.cardCount ? `${product.cardCount} cards` : 'Official 100-card release'}</p>
                </div>
                <div className="flex items-center gap-3 pt-2">
                  {product.scryfall_uri && (
                    <a href={product.scryfall_uri} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 font-mono text-[10px] font-bold uppercase text-primary hover:underline">
                      Scryfall View ↗
                    </a>
                  )}
                </div>
