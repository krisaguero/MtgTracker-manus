      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12">
        ...
        <section className="mt-10 sm:mt-14" aria-labelledby="spike-heading">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            ...
            <p className="text-xs uppercase tracking-wider text-muted-foreground">Showing {visibleSpikes.length} of {spikes.length} indexed movers</p>
          </div>
              ...
              <div key={analysis.deckId} className="border-2 border-border bg-card p-6 flex flex-col justify-between">
                <div>
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`font-mono text-xs font-bold uppercase tracking-wider ${analysis.recommendation === 'BUY_SEALED' ? 'text-emerald-500' : 'text-amber-500'}`}>{analysis.recommendation === 'BUY_SEALED' ? 'Sealed Value Win' : 'Singles Recommended'}</span>
                  </div>
                  ...
                  <ul className="space-y-1.5 font-mono text-xs">
                    {analysis.topSingles.map((single) => (
                      <li key={single.name} className="flex items-center justify-between">
                        ...
                        <span className="text-primary font-bold">${single.price.toFixed(2)}</span>
                      </li>
                    ))}
                  </ul>
                </div>
