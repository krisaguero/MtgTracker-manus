...
export default function CommanderDeck() {
  ...
  const liveMarketValue = useMemo(() => {
    if (!data) return { total: 0, ownedTotal: 0 };
    ...
    for (const entry of allEntries) {
      const price = entry.card.prices?.usd ? Number(entry.card.prices.usd) : 1.5;
      const qty = entry.quantity;
      total += price * qty;
      const ownedQty = ownedMap.get(entry.card.name.toLowerCase()) || 0;
      if (ownedQty > 0) {
        ownedTotal += price * Math.min(ownedQty, qty);
      }
    }
    return { total: Math.round(total), ownedTotal: Math.round(ownedTotal) };
  ...
  const marketContext = getPreconMarketContext([...data.commander, ...data.deck].map((e) => ({ name: e.card.name, usd: e.card.prices?.usd ?? undefined })));
        ...
        <section className="mb-8 border-2 border-primary/30 bg-primary/5 p-5 sm:p-6" aria-labelledby="precon-market-context-heading">
          <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            ...
            <div className="border border-border bg-card p-4 shrink-0">
              ...
              <p className="text-[10px] text-muted-foreground mt-0.5">Tracked in top 1000 spikes</p>
            </div>
          </div>
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {marketContext.highlights.map((highlight: PreconMarketHighlight, idx: number) => (
              <div key={idx} className="border border-border bg-card p-4 flex flex-col justify-between">
                <div>
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-[10px] uppercase font-bold text-primary bg-primary/10 px-1.5 py-0.5">{highlight.cardName}</span>
                    <span className="font-mono text-[10px] font-bold text-emerald-500">+{highlight.weeklyVolatilityPercent}%</span>
                  </div>
        ...
        {groupDeckCards([...data.commander, ...data.deck]).map((group) => (
          <section key={group.label} className="border-2 border-border bg-card p-6 sm:p-8">
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {group.cards.map((entry: { card: ScryfallCard; quantity: number }) => (
                <DeckCardRow key={entry.card.id} entry={{ ...entry, zone: 'deck' }} refreshKey={collectionVersion} arenaMode={arenaMode} />
              ))}
            </div>
          </section>
        ))}
        ...
        {data.commander.length > 1 && (
          <section className="border-t border-border pt-8">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {data.commander.slice(1).map((entry: DecklistEntry) => (
                <DeckCardRow key={entry.card.id} entry={entry} refreshKey={collectionVersion} arenaMode={arenaMode} />
              ))}
            </div>
          </section>
        )}
