...
import { moversForCard, suppliedSignalsForCard } from '@/lib/marketMoverLinks';
import { resolveCardPrice, resolveDeckMarketValue } from '@/lib/marketPriceIndex';

function scryfallFullResolutionImage(card: any): string {
  return card.image_uris?.png || card.image_uris?.large || card.card_faces?.[0]?.image_uris?.png || card.card_faces?.[0]?.image_uris?.large || card.image_uris?.normal || card.card_faces?.[0]?.image_uris?.normal || '';
}
...
export default function CardDetail() {
      ...
      try {
        ...
        if (json && json.data && isMounted) {
          ...
          const versions: PrintingVersion[] = rawCards.map((card: any) => ({
            ...
            imageUrl: scryfallFullResolutionImage(card),
            priceUsd: card.prices?.usd ? Number(card.prices.usd) : (card.prices?.usd_foil ? Number(card.prices.usd_foil) : null),
          ...
          if (versions.length > 0) {
            ...
            const directPrimaryImage = scryfallFullResolutionImage(primaryCard) || preferredPrinting.imageUrl;
  ...
  const activeIndexedPrice = resolveCardPrice(cardNameParam, activeSetCode);
  const activePrice = selectedPrinting?.priceUsd ?? activeIndexedPrice?.usd ?? cardData.price;
  const marketMovers = useMemo(() => moversForCard(cardNameParam, activeSetCode), [cardNameParam, activeSetCode]);
  const suppliedMarketSignals = useMemo(() => suppliedSignalsForCard(cardNameParam), [cardNameParam]);

  const matchingDecks = useMemo(() => {
    const list: Array<{ name: string; setCode: string; setName: string; slug: string; isOwned: boolean; marketValue: number; pricedCards: number; totalCards: number; topValueCard?: { name: string; usd: number } }> = [];
    ...
    for (const deck of commanderDecklistsData) {
      ...
      if (hasCard) {
        ...
        const topValueCard = [...deck.commander, ...deck.cards]
          .map((entry) => {
            const price = resolveCardPrice(entry.name, entry.set_code || deck.set_code);
            return price ? { name: entry.name, usd: price.usd } : null;
          })
          .filter((entry): entry is { name: string; usd: number } => Boolean(entry))
        ...
        list.push({
          ...
          slug,
          isOwned,
          marketValue: marketValue.totalUsd,
          pricedCards: marketValue.pricedCards,
          totalCards: marketValue.pricedCards + marketValue.unpricedCards,
          topValueCard,
        });
      }
    }
      ...
      <main className="mx-auto max-w-7xl w-full px-4 py-8 sm:px-8 flex-1 space-y-10">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className={`self-start border-2 p-6 flex flex-col items-center justify-center bg-card ${cardData.isBrandNewCard ? 'border-amber-500/80 shadow-[0_0_20px_rgba(245,158,11,0.15)]' : 'border-border'}`}>
            ...
            <div className="relative aspect-[5/7] w-full max-w-sm overflow-hidden bg-muted border-2 border-border shadow-lg">
              {activeImage ? (
                <img
                  src={activeImage}
                  alt={cardData.name}
                ...
                <div className="flex h-full items-center justify-center p-4 text-center text-xs text-muted-foreground">Artwork unavailable</div>
              )}
            </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className={`border-2 p-6 sm:p-8 space-y-4 bg-card ${cardData.isBrandNewCard ? 'border-amber-500/60' : 'border-border'}`}>
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <span className="font-mono text-xl font-black text-primary">{activePrice === null ? 'Price unavailable' : `$${activePrice.toFixed(2)} USD`}</span>
              </div>
                      ...
                      >
                        ...
                        <div>
                          ...
                          <div className="flex items-center justify-between font-mono text-[10px] text-muted-foreground mt-1">
                            ...
                            <span className="font-bold text-primary">{p.priceUsd === null ? 'Unavailable' : `$${p.priceUsd.toFixed(2)}`}</span>
                          </div>
                        </div>
                      </button>
                    );
            ...
            <div className="border-2 border-primary/40 bg-card p-6 sm:p-8 space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-3">
                ...
                {marketMovers[0] ? (
                  <span className={`border px-2.5 py-1 font-mono text-xs font-bold ${marketMovers[0].trend === 'up' ? 'border-emerald-500 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'border-rose-500 bg-rose-500/10 text-rose-600 dark:text-rose-400'}`}>
                    {marketMovers[0].trend === 'up' ? '+' : ''}{marketMovers[0].percentChange.toFixed(1)}% snapshot move
                  </span>
                ) : <span className="border border-border px-2.5 py-1 font-mono text-xs text-muted-foreground">No current mover row</span>}
              </div>
              {marketMovers.length > 0 ? (
                <div className="space-y-3">
                  <p className="text-sm text-muted-foreground leading-relaxed">This card appears in the canonical market snapshot. Each row links to the backed-up article post for the specific signal.</p>
                  {marketMovers.slice(0, 3).map((mover) => (
                    <div key={`${mover.setCode}-${mover.category}`} className="flex flex-wrap items-center justify-between gap-3 border border-border bg-background p-3">
                      <div><p className="font-bold text-sm">{mover.category}</p><p className="mt-1 text-xs text-muted-foreground">{mover.reason}</p></div>
                      <Link href={mover.articleHref} className="shrink-0 border border-primary px-2.5 py-1.5 font-mono text-[10px] font-bold uppercase text-primary hover:bg-primary hover:text-primary-foreground">Read mover post</Link>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-sm text-muted-foreground leading-relaxed">This card has no current canonical mover row. The finance panel stays quiet rather than presenting a synthetic trend.</p>
              )}
              {suppliedMarketSignals.length > 0 && (
                <div className="border-t border-border pt-4">
                  <p className="font-mono text-[10px] font-bold uppercase tracking-wider text-primary">Supplied market-watch references</p>
                  {suppliedMarketSignals.map((signal) => (
                    <div key={`${signal.sourceLabel}-${signal.asOf}`} className="mt-2 flex flex-wrap items-center justify-between gap-3 border border-border bg-background p-3">
                      <div><p className="font-bold text-sm">{signal.trendType.replace('-', ' ')}</p><p className="mt-1 text-xs text-muted-foreground">{signal.summary}</p></div>
                      <Link href={signal.articleHref} className="shrink-0 font-mono text-[10px] font-bold uppercase text-primary hover:underline">Read article →</Link>
                    </div>
                  ))}
                </div>
              )}
                  ...
                  {matchingDecks.map((deck) => (
                    ...
                    >
                      ...
                      <div className="mt-4 pt-2 border-t border-border flex items-center justify-between font-mono text-xs">
                        <span className="text-muted-foreground">Indexed equity:</span>
                        <span className="font-bold text-primary">${deck.marketValue.toFixed(2)}</span>
                      </div>
                      <div className="mt-2 flex items-center justify-between font-mono text-[10px] text-muted-foreground">
                        <span>Coverage {deck.pricedCards}/{deck.totalCards}</span>
                        <span>{deck.topValueCard ? `Top: ${deck.topValueCard.name} · $${deck.topValueCard.usd.toFixed(2)}` : 'No top price indexed'}</span>
                      </div>
                    </Link>
                  ))}
                </div>
