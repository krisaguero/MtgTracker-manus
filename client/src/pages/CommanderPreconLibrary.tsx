...
const EXTENDED_PRECON_LIBRARY: PreconLibraryItem[] = commanderDecklistsData.map((deck) => {
  ...
  const msrp = null; // MTGJSON does not publish a sealed MSRP; never infer one from release year.
      ...
      <main className="w-full px-4 py-8 sm:px-6 lg:px-8 2xl:px-12 space-y-8">
        ...
        <div className="mb-8 border-2 border-border bg-card p-6 shadow-sm">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-border">
            <div>
              <div className="flex items-center gap-2 font-mono text-xs font-bold uppercase tracking-widest text-primary mb-1">
                <Sparkles className="h-4 w-4" /> Commander Market Valuation Matrix
              </div>
              <h2 className="text-xl font-extrabold tracking-tight">Full Product Pricing &amp; Value Breakdown</h2>
              <p className="font-mono text-xs text-muted-foreground mt-1">
                Aggregating current per-card market prices across {EXTENDED_PRECON_LIBRARY.length} Commander products. Sealed MSRP is shown only when a dated source exists; this catalog does not infer retail prices.
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-3 font-mono text-xs">
              <div className="border border-border bg-background px-3 py-2">
                <span className="text-muted-foreground block text-[10px] uppercase">Total Catalog Market Equity</span>
                <span className="font-black text-sm text-primary">
                  ${EXTENDED_PRECON_LIBRARY.reduce((sum, d) => sum + resolveDeckMarketValue(d).totalUsd, 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USD
                </span>
              </div>
              <div className="border border-border bg-background px-3 py-2">
                <span className="text-muted-foreground block text-[10px] uppercase">Average Deck Value</span>
            ...
            <table className="w-full text-left font-mono text-xs">
              <thead>
                <tr className="border-b border-border text-muted-foreground uppercase text-[10px]">
                  <th className="py-2.5 px-3">Commander Deck</th>
                  <th className="py-2.5 px-3">Set</th>
                  <th className="py-2.5 px-3">Indexed Equity</th>
                  <th className="py-2.5 px-3">Street MSRP</th>
                  <th className="py-2.5 px-3">Value Spread</th>
                  <th className="py-2.5 px-3">Price Coverage</th>
              ...
              <tbody className="divide-y divide-border/60">
                {EXTENDED_PRECON_LIBRARY.map((deck) => {
                  ...
                  const spread = deck.msrp === null ? null : val.totalUsd - deck.msrp;
                  const isProfit = spread !== null && spread >= 0;
                  return (
                    <tr key={`matrix-${deck.slug}`} className="hover:bg-muted/50 transition-colors">
                      <td className="py-2.5 px-3 font-bold text-foreground max-w-[240px] truncate">{deck.name}</td>
                      <td className="py-2.5 px-3 uppercase text-muted-foreground">{deck.set_code}</td>
                      <td className="py-2.5 px-3 font-black text-primary">${val.totalUsd.toFixed(2)}</td>
                      <td className="py-2.5 px-3">{deck.msrp === null ? <span className="text-muted-foreground">Unavailable</span> : `$${deck.msrp.toFixed(2)}`}</td>
                      <td className={`py-2.5 px-3 font-bold ${spread === null ? 'text-muted-foreground' : isProfit ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400'}`}>
                        {spread === null ? 'Needs sealed baseline' : isProfit ? `+$${spread.toFixed(2)}` : `-$${Math.abs(spread).toFixed(2)}`}
                      </td>
                      <td className="py-2.5 px-3 text-muted-foreground">{val.coveragePercent}% ({val.pricedCards} cards)</td>
                      <td className="py-2.5 px-3 text-right">
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => navigate(`/deck/${deck.set_code}/${deck.slug}`)}
                ...
                <div>
                  ...
                  <div className="relative aspect-[16/9] w-full overflow-hidden bg-background border-b-2 border-border">
                    ...
                    <div className="absolute top-3 left-3 right-3 z-20 flex flex-wrap items-center justify-between gap-2 pointer-events-none">
                      ...
                      <span className="border-2 border-primary bg-primary text-primary-foreground px-2.5 py-0.5 font-mono text-right font-black shadow-md">
                        <span className="block text-xs">{marketValue.coveragePercent > 0 ? `~$${marketValue.totalUsd} USD` : 'PRICE INDEX PENDING'}</span>
                        {marketValue.coveragePercent > 0 && <span className="block text-[9px] font-bold tracking-wider text-primary-foreground/80">{marketValue.coveragePercent}% INDEXED</span>}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="p-5 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-2 border-t border-border font-mono text-xs">
                      <div>
                        ...
                        <span className="font-bold">{deck.msrp === null ? 'Unavailable' : `$${deck.msrp.toFixed(2)}`}</span>
                      </div>
